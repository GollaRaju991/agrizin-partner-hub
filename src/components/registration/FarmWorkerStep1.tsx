    ...
    <div className="space-y-5 pb-24">
      ...
      <div className="bg-card rounded-2xl border border-border p-4 shadow-card space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div>
            <Label className="text-sm font-medium text-foreground mb-2 block">{t("gender")} *</Label>
            <RadioGroup
              value={data.gender}
            ...
            >
              {[
                ...
                <div key={g.v} className="flex items-center gap-1.5">
                  <RadioGroupItem value={g.v} id={`gender-${g.v}`} />
                  <Label htmlFor={`gender-${g.v}`} className="text-sm text-foreground cursor-pointer">{g.label}</Label>
                </div>
              ))}
            </RadioGroup>
          ...
          <div>
            <div className="flex items-center gap-3">
              <Label className="text-sm font-medium text-foreground whitespace-nowrap">{t("age")} *</Label>
              <Input
                type="number"
                placeholder="25"
                value={data.age}
      ...
      <div className="bg-card rounded-2xl border border-border p-4 shadow-card space-y-3">
        ...
        <div className="flex items-center justify-between border border-border rounded-xl px-3 h-11">
          ...
          <span className="text-sm text-muted-foreground">{t("india")}</span>
        </div>
        ...
        <div className="pt-1">
          ...
          >
            {locating ? (
              <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> {t("gettingLocation")}</>
            ) : data.latitude && data.longitude ? (
              <><Navigation className="w-4 h-4 mr-2" /> {t("gpsCaptured")} ({data.latitude.toFixed(4)}, {data.longitude.toFixed(4)})</>
            ) : (
              <><Navigation className="w-4 h-4 mr-2" /> {t("useCurrentLocation")}</>
            )}
          </Button>
